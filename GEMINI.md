...
- **Runner Dependencies:** Since the default runner pod image (`ghcr.io/actions/actions-runner:latest`) is extremely minimal and lacks runtime tools, the workflow [gitops.yml](file:///.github/workflows/gitops.yml) bootstraps `go-task`, `helm`, `kubectl`, and `helmfile` using `azure/setup-helm`, `azure/setup-kubectl`, and architecture-aware `curl` scripts with automatic retries (`--retry 5`) for `go-task` and `helmfile` to support ARM64 Raspberry Pi nodes and prevent transient download timeouts.
