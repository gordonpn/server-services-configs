...
- **Alertmanager Inhibition Rules:** Alertmanager is configured with `inhibit_rules` in `values-monitor.yaml` to suppress downstream symptoms (such as `KubePodNotReady`, `TargetDown`, and `KubeDeploymentReplicasMismatch`) whenever root-cause alerts (`KubeNodeNotReady`, `KubeletDown`, or `KubeAPIDown`) are firing for that node/cluster.
