...
- **Longhorn Data Locality & Prometheus CrashLoop Prevention:** Continuous-write stateful workloads (like Prometheus TSDB) pinned to `master` must be configured with `dataLocality: best-effort` so the primary active replica runs locally on `master`. When `dataLocality` is disabled, Longhorn may place the active replica exclusively on worker nodes (such as `pi-bos-0` or remote Montreal nodes). High CPU or network contention on worker nodes then triggers Longhorn backend I/O timeouts (`r/w timeout`), reporting `no backend available` and surfacing kernel I/O errors (`EIO`) on memory-mapped active files (`/prometheus/queries.active`). This causes Prometheus to crash in a panic loop and trigger continuous `KubePodCrashLooping` and `TargetDown` alert storms.
