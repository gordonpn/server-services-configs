...
- **Longhorn Master-Pinned StorageClass & TSDB Strict Locality:** Continuous-write stateful workloads (like Prometheus TSDB and Loki) pinned to `master` must use the `longhorn-master` StorageClass (`numberOfReplicas: 1`, `dataLocality: strict-local`, and `nodeSelector: master` matching the `master` tag in `nodes.longhorn.io`). Under multi-replica or `best-effort` locality, Longhorn schedules replicas across worker nodes (such as `pi-bos-0`). Streaming multi-gigabyte block snapshots across Tailscale WireGuard pegs Raspberry Pi CPUs at 100%, causing HTTP ssync header timeouts (`Client.Timeout exceeded while awaiting headers`), unending rebuild loops, and kernel `EIO` errors (`open /prometheus/queries.active: input/output error`) that trigger crash loops and alert storms. `strict-local` ensures zero remote replication overhead and 100% local NVMe/SSD speeds on `master`.
