...
- **Alertmanager Inhibition Rules & Synthetic Alert Routing:** Alertmanager is configured with `inhibit_rules` in `values-monitor.yaml` to suppress downstream symptoms (such as `KubePodNotReady`, `TargetDown`, and `KubeDeploymentReplicasMismatch`) whenever root-cause alerts (`KubeNodeNotReady`, `KubeletDown`, or `KubeAPIDown`) are firing. Internal synthetic rules generated by Prometheus Operator (`Watchdog` and `InfoInhibitor`) as well as benign backup-time bursts (`CPUThrottlingHigh` and `NodeMemoryMajorPagesFaults`) must be explicitly routed to the `null` receiver (`match_re: alertname: ^(Watchdog|InfoInhibitor|CPUThrottlingHigh|NodeMemoryMajorPagesFaults)$`) to prevent spamming notifications.
...
- **Uptime Kuma Infrastructure as Code (Terraform):** All push monitors and notifications are managed via Terraform in `terraform/uptime_kuma.tf` using the `breml/uptimekuma` provider. Push monitors must match live configuration (`interval = 120`, `retry_interval = 120`, `resend_interval = 1`, `max_retries = 2`, `notification_ids = [uptimekuma_notification_slack.slack.id]`) to prevent state drift and token churn. Sensitive variables are configured via gitignored `terraform.tfvars` or `TF_VAR_` environment variables.
...
- **Longhorn Automated Recurring Backups via Local MinIO:** To provide asynchronous durability for single-replica workloads on `master` without synchronous block replication overhead, a lightweight MinIO instance runs in `longhorn-system` backed by a hostPath mount to `/media/storage/backups` on the attached 1TB SSD of `master`. Longhorn's `default` BackupTarget is configured to `s3://longhorn-backup@us-east-1/` using credentials in `longhorn-backup-secret`. A daily `RecurringJob` (`daily-backup`) runs at 03:00 UTC with 7-day retention across the `default` volume group, saving compressed and deduplicated block snapshots directly to local attached storage. MinIO is provisioned with 2000m CPU limits and 1Gi memory to prevent CFS throttling and speed up nightly backup streaming.
- **Short-Interval CronJob Image Pull Policy & Restart Policy:** Recurring CronJobs running at high frequencies (such as `lectures-notifier-et-10m` every 5 minutes) must specify `imagePullPolicy: IfNotPresent` and `restartPolicy: Never`. Using `imagePullPolicy: Always` forces container runtimes to pull and unpack Docker image layers continuously on worker nodes, causing root USB disk queue saturation (`NodeDiskIOSaturation`) and spiking system load (`NodeSystemSaturation`) on low-power nodes like Raspberry Pis. Using `restartPolicy: Never` ensures failed runs terminate cleanly and wait for the next scheduled interval, avoiding in-pod restart loops, pod churn, and SIGTERM contention.
...
- **Terraform Deploy on Push (GitOps Automation):** Terraform changes currently execute manually via `task tf:plan` and `task tf:apply` against a local state file. Automated execution on push ([.github/workflows/terraform.yml](file:///.github/workflows/terraform.yml)) is tracked as a to-do in [iac_audit_and_roadmap.md](file:///docs/iac_audit_and_roadmap.md). It requires migrating state to a remote backend (such as MinIO S3 or Cloudflare R2), configuring repository secrets (`CLOUDFLARE_API_TOKEN`, `TF_VAR_kuma_password`, `TF_VAR_kuma_slack_webhook_url`), scoping path triggers to `terraform/**`, and removing the `if: false` disable flag.
