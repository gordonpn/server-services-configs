...
- **IPv4 Precedence & IPv6 Egress:** Self-hosted runner pods in K3s lack IPv6 egress routing. When external services (such as GitHub OCI registries like `pkg-containers.githubusercontent.com`) publish AAAA records, tools (`helm`, `go-task`, `curl`) attempting IPv6 connections fail with `connect: network is unreachable`. The GitOps workflow configures `/etc/gai.conf` precedence (`precedence ::ffff:0:0/96 100`) and disables kernel IPv6 (`net.ipv6.conf.all.disable_ipv6=1`) to force IPv4 routing.
