...
- **Short-Interval CronJob Image Pull Policy:** Recurring CronJobs running at high frequencies (such as `lectures-notifier-et-10m` every 5 minutes) must specify `imagePullPolicy: IfNotPresent`. Using `imagePullPolicy: Always` forces container runtimes to pull and unpack Docker image layers continuously on worker nodes, causing root USB disk queue saturation (`NodeDiskIOSaturation`) and spiking system load (`NodeSystemSaturation`) on low-power nodes like Raspberry Pis.
